<rg-date>
	...
	<div class="c-container">
		...
		<div class="c-calendar c-calendar--high" if="{ opts.date.isvisible }">
			...
			<button class="c-calendar__date c-calendar__date--in-month { 'calendar__date--selected': day.selected, 'c-calendar__date--today': day.today }" disabled="{ day.disabled }" each="{ day in days }" onclick="{ select }">{ day.date.format(dayFormat) }</button>
			...
			<button class="c-button c-button--block c-button--primary" disabled="{ opts.date.min.isAfter(moment(), 'day') || opts.date.max.isBefore(moment(), 'day') }" onclick="{ setToday }">Today</button>
		</div>
	</div>
